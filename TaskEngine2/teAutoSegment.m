...
% filter log for trial change events
    ...
    tab = teLogFilter(data.Log, 'topic', 'trial_change');
...
% on/offset timestamps from the log
    ...
    bigLogTable = teLogFilter(data.Log, 'topic', 'trial_log_data');
    bigEventTable = teLogFilter(data.Log, 'source', 'teEventRelay_Log');
...
%             continue
    ...
    % look for eye tracking data. If found, segment for this trial
        ...
        if hasET(t)
            ...
            s1 = teTimeToSample(data.EyeTracking.Buffer, onset, 0.010);
            s2 = teTimeToSample(data.EyeTracking.Buffer, offset, 0.010);
            ...
            if isempty(s2)
                s2 = size(data.EyeTracking.Buffer, 1);
            end
            ...
            if isempty(s1) || isempty(s2)
                ...
                % segment
                trials{t}.EyeTracking = data.EyeTracking.Buffer(s1:s2, :);
