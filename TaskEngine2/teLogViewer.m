classdef teLogViewer < handle
    ...
    methods
        ...
        function tab = PrepareTableData(obj)
            tab = table2cell(obj.prTable);
            idx_wrong = cellfun(@(x) ~iscell(x) && ~isnumeric(x) &&...
                ~islogical(x) && ~ischar(x), tab(:));
            tab(idx_wrong) = repmat({'<CANNOT DISPLAY>'}, sum(idx_wrong), 1);
        end
