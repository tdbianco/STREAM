classdef teAnalysisClient < teAnalysisDatabase
...
%             ops{oc} = struct;
        ...
        function [suc, ops, guid] = IngestMetadata(obj, md)
        % takes a pre-prepared teMetadata and ingests it
            ...
            % check that .Paths collection is empty (cannot ingest metadata
            % with existing external data paths, since these by definition
            % refer to relative paths inside a database)
            if ~isempty(md.Paths)
                error('Cannot ingest metadata unless .Paths collection is empty.')
            end
