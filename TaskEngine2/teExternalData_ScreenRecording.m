    ...
    methods
        ...
        function obj = teExternalData_ScreenRecording(path_screen)
            ...
            if ~exist('path_screen', 'var') || isempty(path_screen)
                error('Must supply a path to screen recording data.')
        ...
        % look for compatible video files
            ...
            if isempty(file_video)
                % no files found
                warning('No compatible video file not found in screenrecording folder.')
                return    
                
            elseif length(file_video) > 1
                % more than one files found - can't load in this instance
                warning('More than one video file fount in screenrecording folder.')
                return
        ...
        % read video metadata
            ...
            try
                ...
                warning('Error reading video metadata, error was:\n\n%s',...
                    ERR.message)
            end
        ...
        % look for sync struct
            ...
            file_sync = teFindFile(path_screen, sprintf('*%s*sync.mat', fil));
