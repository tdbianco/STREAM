...
%             obj.UpdatePosixOffset;
        ...
        % init PTB graphics
            ...
            % set resolution of mono screen preview to be screen res scaled
            % down (to ensure that the aspect ratio is correct)
            obj.calculateMonoScreenResolution
...
%             end
        ...
        function [flipTime, flipTimeLocal] = RefreshDisplay(obj, varargin)
            ...
            doKeyUpdate = ~ismember('noKeyUpdate', varargin);
...
%             obj.AddLog('topic', 'debug', 'data', 'Finished housekeeping')
            tm(5) =  teGetSecs * 1000;
            % keyboard
            if doKeyUpdate, obj.KeyUpdate, end
...
%             end
        ...
        function DrawTaskDetails(obj)
            
            % clear texture
            obj.ClearTexture(obj.prTaskDetailsTexture)
            
            % set font size
            prevTextSize = Screen('TextSize', obj.prTaskDetailsTexture, 50);
            
            % build task/trial number strings
            if ~isempty(obj.prCurrentTask)
                str_task = sprintf('Task: %s', obj.prCurrentTask);
            else
                str_task = 'Task: none';
            end
            if ~isempty(obj.prCurrentTrial)
                str_trial = sprintf('Trial: %d', obj.prCurrentTrial);
            else
                str_trial = 'Trial: n/a';
            end
            
            % cat strings
            str = sprintf('%s | %s', str_task, str_trial);
            
            % get size of text
            bounds = Screen('TextBounds', obj.prTaskDetailsTexture, str);
            tw = bounds(3);
            th = bounds(4);
            ...
            % draw white with black shadow for readability
%             Screen('DrawText', obj.prTaskDetailsTexture, str, x + 2, y + 2,...
%                 [000, 000, 000, 255]);
            Screen('DrawText', obj.prTaskDetailsTexture, str, x, y,...
                [255, 255, 255, 255], [000, 000, 000, 50]);
            
            % restore font size
            Screen('TextSize', obj.prTaskDetailsTexture, prevTextSize);
...
%             Screen('FillRect', obj.prWindowPtr, obj.prBackColour);
        ...
        function DrawStim(obj, stimName, rect, angle, alpha, immediate)
            ...
            elseif ischar(stimName)
                ...
                if isempty(stim)
                    error('Stimulus name %s not found.', stimName)
                end                
            else
...
%             end
            % if screen is not open, or monitor size has not been set, we 
            % can't calculate scaling params, so note this and return
            if isempty(obj.MonitorNumber) || isempty(obj.prMonitorSize)
                obj.prScalingValid = false;
                return
            end

            % get resolution 
            res = Screen('Rect', obj.MonitorNumber);
            res = res([3, 4]);
            % calculate params
            ...
            obj.AddLog(...
                'source',       'presenter',...
                'topic',        'scaling',...
                'cm_per_px',    obj.prCmPerPx,...
                'px_per_cm',    obj.prPxPerCm);
        end
        ...
        function OpenWindow(obj)
            ...
            obj.prTaskDetailsTexture = Screen('OpenOffscreenWindow',...
                obj.prWindowPtr, [0, 0, 0, 0], res);            
            ...
            Screen('BlendFunction', obj.prTaskDetailsTexture, GL_SRC_ALPHA,...
                GL_ONE_MINUS_SRC_ALPHA);   
            ...
            Screen('FillRect', obj.prWindowPtr, obj.BackColour)
            obj.DrawBackColour
            obj.RefreshDisplay;
...
%             Screen('FinalizeMovie', obj.prCapturePtr);
        ...
        % enable auto off in future
            ...
            % check enabled
            if ~obj.LightPatchEnabled
                warning('Light patch not enabled, set LightPatchEnabled to true before calling this method.')
                return
            end
        ...
        function ETGetEyesAndCalibrate(obj)
            ...
            % temp disable "NOT LOOKING" message so that it doesn't
            % interfere with calibration results on preview window. Store
            % this setting in a variable to reapply at the end of the
            % method
            oldDrawGazeOffscreenMessage = obj.DrawGazeOffscreenMessage;
            obj.DrawGazeOffscreenMessage =false;
            ...
            % restore setting for drawing "NOT LOOKING" message to whatever
            % it previously was
            obj.DrawGazeOffscreenMessage = oldDrawGazeOffscreenMessage;
...
%                 tdeg    = char(176);
        ...
        function SwitchKeyboard(obj, assignedName)
            ...
            if isempty(dev)
                ...
                obj.AddLog(...
                    'source',           'presenter',...
                    'topic',            'keyboard_change',...
                    'activekb_idx',     obj.prActiveKeyboard,...
                    'activekb_name',    assignedName);
            end 
        end
...
%             % keyboard input can be disabled (mostly for testing purposes)
%             if obj.DisableKeyboard
%                 return
%             end
        ...
        function [comp, map, taskKey, trialCounter] =...
                CompileList(obj, val, level, trialCounter, numSamples,...
                startSample)
            ...
            % if numSamples is specified, then we just want that number
            % from the list. This allows nested lists to be called with a
            % specific number of samples. If numSampes is not specified
            % then use the .Numsamples properly of the target list. 
            if ~exist('numSamples', 'var') || isempty(numSamples)
                numSamples = list.NumSamples;
            end
            if ~isnumeric(numSamples) || ~isscalar(numSamples) ||...
                    numSamples < 1
                error('numSamples must be a positive numeric scalar.')
            end
            
            if ~exist('startSample', 'var') || isempty(startSample)
                startSample = list.StartSample;
            end
            if ~isnumeric(startSample) || ~isscalar(startSample) ||...
                    startSample < 1
            ...
            % to be different each time it is called. This is set in each
            % list, so block lists that expect to be called more than once
            % will have the RecalculateEachTime property set to true.
            % Otherwise we don't recalculate, beacuse the order should
            % remain stable across a session. 
            if list.RecalculateSamplingEachTime
                list.CalculateSampling;
            end
            
            % based upon startSample and numSamples, calculate the list
            % items to compile
            s1 = startSample;
            s2 = startSample + numSamples - 1;
            if s1 > list.NumSamples
                error('Start sample of %d is beyond the size of the list (%d)',...
                    s1, list.NumSamples)
            elseif s2 > list.NumSamples
                error('End sample of %d is beyond the size of the list (%d)',...
                    s2, list.NumSamples)
            elseif s1 < 1
                error('Start sample must be >= 1.')
            elseif s2 < 1
                error('End sample must be >= 1.')
            end
...
%                     end
                    ...
                    % check number of samples in list item. If not
                    % specified then set to empty, which will cause the
                    % CompileListItem method to use the .NumSamples
                    % property of the target list
                    if ~isnan(sample.NumSamples) &&...
                            ~isempty(sample.NumSamples)
                        numSamples = sample.NumSamples;
                    else
                        numSamples = [];
                    end
                    
                    if ~isnan(sample.StartSample) &&...
                            ~isempty(sample.StartSample)
                    ...
                    [comp, map, taskKey, trialCounter] =...
                        ...
                        trialCounter, numSamples, startSample);
                    % add parent vars
...
%                     tc = tc + 1;
        ...
        function ExecuteList(obj, val, startSample, numSamples)
            ...
            % occurred
                ...
                obj.prCurrentTrial = thisTask.TrialNo;   
...
%                             vars(it).Presenter_TrialNo = thisTask.TrialNo;
                        end
                    end
                ...
                % clear AOIs between trials, unless the task has marked
                % this as unwanted
                if thisTask.ClearAOIsOnTrialEnd
                    obj.EyeTracker.AOIs.Clear;
                end
...
%                 match = find(ismember(relay_name, types));
        ...
        % general checks
            ...
            % if stamping video, do it now. This will cause a SYNC event to
            % be sent to all event relays. If stamping the video this is
            % what we want, so that the SYNC corresponds to the QR code in
            % the video. If NOT stamping the video, then we still want to
            % send the sync event
            if obj.StampVideoOnSessionStart
                ...
                % send SYNC event (which normally happens when syncing the
                % video)
                obj.SendRegisteredEvent('SYNC', teGetSecs);
            end
...
%                 s{i}.timestamp = s{i}.timestamp + obj.PosixTimeOffset;
            ...
            for i = 1:numItems
                ...
                % optionally echo to command window
                if obj.EchoLog
                    disp(s{i})
                end
            end
...
%                 ':CodecType=ffenc_ljpeg');
        ...
        function set.MonitorNumber(obj, val)
            ...
            if obj.prMonitorNumber == 0 && ~obj.MonoScreenMode
                teLine
                teEcho('You have set MonitorNumber to 0. This will open a window\nfullscreen on your main monitor. Do you want to set MonoScreenMode\nin order to present stimuli in a small window (y/n)? >');
                resp = input('', 's');
                if instr(lower(resp), 'y')
                    obj.MonoScreenMode = true;
                end
            end
            % recalc mono screen resolution, to ensure we get the aspect
            % ratio correct (i.e. not for a monitor that may have
            % previously been set but which is now being changed)
            obj.calculateMonoScreenResolution
        ...
        function set.SkipSyncTests(obj, val)
            ...
            obj.AddLog(...
                'source',           'presenter',...
                'topic',            'synctests',...
                'skipsynctests',    obj.prSkipSyncTests);
        end
        ...
        function set.WindowLimitEnabled(obj, val)
            ...
            % update eye tracker with new window limits
            obj.EyeTracker.UpdateWindowLimit(obj.prWindowLimitEnabled,...
                obj.MonitorSize, obj.prWindowLimitSize)
            % log
            obj.logWindowLimits
        end
        ...
        function set.WindowLimitSize(obj, val)
            ...
            elseif isempty(obj.MonitorSize)
               ...
               f % applied, since window limits aren't allowed to go beyond
...
%                     end
            ...
            function btnDelCol_Click(h, eventdata)
                ...
                try
                    list.DeleteVariable(tbl_selCol - 1)
                catch ERR
                    rethrow ERR
                end            
                displayList
...
%             res_drawingWindow = Screen('Rect', obj.MonitorNumber);
            res_drawingWindow = obj.ScalePoint(obj.DrawingSize, 'cm2px');
            res_previewWindow = Screen('Rect', obj.PreviewMonitorNumber);
            % calculate width and height
            w = round(res_drawingWindow(1) * scale);
            h = round(res_drawingWindow(2) * scale);
...
%             warning('This is slow!')
    ...
    methods (Hidden)
        ...
        function calculateMonoScreenResolution(obj)
            fullScreenRes = Screen('Rect', obj.MonitorNumber);
            obj.MonoScreenResolution = fullScreenRes(3:4) * .3;
        end
...
%             obj.DrawStim(varargin{:})
        ...
        % logging 
        function logWindowLimits(obj)
            obj.AddLog(...
                'source',       'presenter',...
                'topic',        'windowlimits',...
                'wl_enabled',   obj.prWindowLimitEnabled,...
                'wl_size',      obj.prWindowLimitSize,...
                'wl_bordercol', obj.WindowLimitBorderColour,...
                'drawingsize',  obj.DrawingSize);
        end
        
        function logMonitor(obj)
            obj.AddLog(...
                'source',               'presenter',...
                'topic',                'monitor',...
                'mon_number',           obj.prMonitorNumber,...
                'mon_size',             obj.prMonitorSize,...
                'mon_res',              obj.Resolution,...
                'mon_monoscreenmode',   obj.MonoScreenMode,...
                'drawingsize',  obj.DrawingSize);
        end
