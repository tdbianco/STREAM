...
%% Save to tidy format for analysis

tidy_290 = table;
    
for ss = 1:height(stream_Faces_features)
    ntrl = [stream_Faces_features.Nrantrls{ss}]';
    id = repmat({stream_ClnEEG.IDses{ss}},1,length(ntrl))';
    n290lat = [stream_Faces_features.N290lat{ss}]';
    n290mamp = [stream_Faces_features.N290mamp{ss}]';
    tidy_290_temp = table(id, ntrl, n290lat, n290mamp);
