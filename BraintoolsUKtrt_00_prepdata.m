...
for ii = 1:height(stream_IDs_tracking_wETint_MW)
    % number of EEG events
    xxxE = stream_IDs_tracking_wETint_MW.tNev_EEG{ii};
    MW_EEGn(ii,1) = sum(xxxE,2);
end
clear xxxE 
    
Ntrls_EEGIN = IN_EEGn(xxxin,1);
Ntrls_EEGMW = MW_EEGn(xxxmw,1);
figure;
h1 = histogram(Ntrls_EEGMW);
hold on
h2 = histogram(Ntrls_EEGIN);
h1.BinWidth = 17;
h2.BinWidth = 6;
legend('India','Malawi')
ylabel('N participants with ET > 60%')
xlabel('N EEG trials')

IDs_HighlyAttentiveET_IN = stream_IDs_tracking_wETint_IN.ID(xxxin);
IDs_HighlyAttentiveET_MW = stream_IDs_tracking_wETint_MW.ID(xxxmw);
